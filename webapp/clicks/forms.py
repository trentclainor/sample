...
class ClicksFilterForm(Form):
    link = TextField(_(u'Сссылка перехода'), validators=[Optional()], description=u'регулярное выражение')
    ...
    conv_status = SelectField(_(u'Статус конверсии'), validators=[Optional()], coerce=str)
    status = SelectField(_(u'Статус перехода'), validators=[Optional()], coerce=str)
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ClicksFilterForm, self).__init__(*args, **kwargs)
        ...
        self.conv_status.choices = ANY_STATUS + NO_STATUS + CONVERSATION_FILTER_STATUSES
        self.status.choices = ANY_STATUS + CLICK_FILTER_STATUSES
