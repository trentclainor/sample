import operator

from rest_framework import status
from rest_framework.response import Response
from rest_framework.reverse import reverse
from rest_framework.viewsets import ViewSet

from sample.common.models import (City, Country, ExternalRole, ExternalSkill, Industry, JOB_TYPES, LANG_LEVELS,
                                     MESSAGE_STATUSES, Role, State)


class ChoicesViewSet(ViewSet):
    http_method_names = ['get', ]
    choices_map = {
        'countries': Country.objects.values_list('id', 'name'),
        'states': State.objects.values_list('id', 'name'),
        'cities': City.objects.values_list('id', 'name'),
        'external-roles': ExternalRole.objects.values_list('title_id', 'title'),
        'external-skills': ExternalSkill.objects.values_list('skill_id', 'skill'),
        'industries': Industry.objects.values_list('id', 'name'),
        'job_types': JOB_TYPES,
        'lang_levels': LANG_LEVELS,
        'message_statuses': MESSAGE_STATUSES,
        'roles': Role.objects.values_list('id', 'name'),
    }

    @staticmethod
    def format_item(item):
        k, v = item
        return {'value': k, 'title': str(v)}

    def prepare(self, choices):
        return list(map(self.format_item, choices))

    @staticmethod
    def search_by_title(choices, search):
        if search is not None:
            choices = filter(
                lambda x: search.lower() in x.get('title').lower(),
                choices
            )
        return list(choices)

    @staticmethod
    def order_by_title(choices):
        if choices:
            choices.sort(key=operator.itemgetter('title'))
        return choices

    def retrieve(self, request, pk=None):
        """
        Return data of choices field by {id}.
        - id - choices field name.
        """
        search = request.GET.get('search')

        choices = self.choices_map.get(pk)
        if choices:
            choices = self.prepare(choices)
            choices = self.search_by_title(choices, search)
            choices = self.order_by_title(choices)
            return Response(choices, status=status.HTTP_200_OK)
        return Response(status=status.HTTP_404_NOT_FOUND)

    def list(self, request):
        """
        Return names of choices fields.
        """
        data = {}
        for k, v in self.choices_map.items():
            url = reverse('choices-detail', args=[k], request=request)
            data[k] = url
        return Response(data, status=status.HTTP_200_OK)
